 ...
 */
export const eachMenuTree = (treeDatas: any[], callBack: Fn, parentPath = '') => {
  treeDatas.forEach((element) => {
    callBack(element, parentPath)
    if (element.children) {
      let nextParentPath = ''
      if (parentPath) {
        nextParentPath = parentPath + '/' + element.path
      } else {
        nextParentPath = element.path
      }
      eachTree(element.children, callBack, nextParentPath)
    }
  })
}
