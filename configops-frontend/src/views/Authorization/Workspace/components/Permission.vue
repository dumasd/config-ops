...
import { getWorkspacePermissionApi } from '@/api/admin'
import { PropType, ref, unref } from 'vue'
import { WorkspaceItem, GroupPermissionsItem } from '@/api/admin/types'
...
const permissionTemplate = ref([
  ...
  {
    module: 'WORKSPACE_WORKER_MANAGE',
    checkList: [''],
    actions: ['READ', 'CREATE', 'EDIT', 'DELETE', 'DETAIL']
  },
  {
    module: 'WORKER_MANAGED_OBJECT_MANAGE',
    checkList: [''],
    actions: ['READ']
  },
  {
    module: 'MANAGED_OBJECT_PERMISSION_MANAGE',
    checkList: [''],
    actions: ['READ', 'EDIT']
  }
])
...
getWorkspacePermissionApi(props.currentRow?.id).then((res) => {
