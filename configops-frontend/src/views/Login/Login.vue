...
import { oidcInfoApi } from '@/api/login'
...
const loginMode = ref('NONE')

const oidcInfoProp = ref({})

oidcInfoApi().then((res) => {
  if (res.data.enabled) {
    if (res.data.auto_login) {
      loginMode.value = 'OIDC_AUTO_LOGIN'
      window.location.href = res.data.sso_url
    } else {
      loginMode.value = 'OIDC_LOGIN'
    }
  } else {
    loginMode.value = 'NORMAL'
  }
  oidcInfoProp.value = res.data
})
</script>
...
<template>
  ...
  >
    <ElScrollbar class="h-full" v-if="loginMode == 'NORMAL' || loginMode == 'OIDC_LOGIN'">
